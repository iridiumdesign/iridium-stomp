...
- **BREAKING — `SubscriptionOptions` gains two public fields, `channel_capacity` and `overflow_limit`**, so a struct literal naming only `headers` no longer compiles. Add `..Default::default()`, or build it with `SubscriptionOptions::default().headers(...)`; see Added (#115)

### Added

- `SubscriptionOptions::channel_capacity` sets the size of a subscription's channel, which was fixed at 16. `None`, the default, means `SubscriptionOptions::DEFAULT_CHANNEL_CAPACITY` (still 16), which `subscribe` and `subscribe_with_headers` also use; zero is treated as one. `SubscriptionOptions::headers` and `SubscriptionOptions::channel_capacity` are builder methods for the two fields (#115)
