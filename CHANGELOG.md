...
- **BREAKING — `SubscriptionOptions` gains a public field, `channel_capacity`**, so a struct literal naming only `headers` no longer compiles. Add `..Default::default()`, or build it with `SubscriptionOptions::default().headers(...)`; see Added (#115)
